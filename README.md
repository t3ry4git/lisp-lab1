...
| `last`           | Повертає останній елемент списку(підсписок [car:cdr]) | `(car (last '(a b c))) ;; => c`              |
...
В результаті уточнення, треба рахувати такі випадки як список з одним елементом.
ВАЖЛИВО!!!
Якщо у структурі є два або більше підсписків з однаковим символом у CAR, обов'язково треба створювати новий підсписок, а не використовувати елемент іншого підсписку. Чи навпаки, у випадку якщо посилання йде на один елемент з двох або більше CAR, не можна створювати більше одного елементу. 
Хоча й результуючі списки виглядають однаково, але структурно в них є різниця, кількість елементів РІЗНА!
(setq in-list '(a) my-list (list in-list 'a)) - тут у купі 3 елементи, а саме A з in-list, (A) з my-list, A з my-list.
(setq in-list '(a) my-list (cons in-list in-list)) - тут у купі 2 елементи, а саме A з in-list, (A) з in-list, A з in-list.
Обидва виводять один результат: ((A) A) 
